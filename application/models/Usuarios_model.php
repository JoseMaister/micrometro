...
public function userCots() {
       $this->db->select('p.*, concat(u.nombre, " ", u.paterno) as name, u.id as idus');
    ...
    $this->db->select('no_empleado');
    $this->db->from('usuarios');
    $this->db->like('no_empleado', 'MC-', 'after');
    $this->db->order_by('no_empleado', 'DESC');
    $this->db->limit(1);
    
    $query = $this->db->get();
    
    if ($query->num_rows() > 0) {
        return $query->row()->no_empleado;
    } else {
        return null;
    }
}
