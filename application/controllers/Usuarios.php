...
class Usuarios extends CI_Controller {
    ...
    function catalogo() {
        $this->load->view('header');
        $this->load->view('usuarios/catalogo_clientes');
    }
...
//echo var_dump($datos['privilegio']);die();
      $this->load->view('header');
    ...
    function alta() {
     ...
     function alta_cliente() {
        $this->load->view('header');
        $this->load->view('usuarios/alta_clientes');
    }
public function registrar() {
    $this->load->model('privilegios_model');
    $this->load->model('usuarios_model'); // Asegúrate de cargar el modelo de usuarios
    $ACIERTOS = array();
    $ERRORES = array();

    // Obtener el último código
    $ultimo_codigo = $this->usuarios_model->obtener_ultimo_codigo();
    
    // Calcular el nuevo código
    if ($ultimo_codigo) {
        $num = (int)substr($ultimo_codigo, 3); // Extrae el número después de "mc-"
        $nuevo_num = $num + 1;
    } else {
        $nuevo_num = 1;
    }
    
    $nuevo_codigo = "MC-" . str_pad($nuevo_num, 3, "0", STR_PAD_LEFT);

    $data = array(
        'no_empleado' => $nuevo_codigo,  // Agregamos el nuevo código
        ...
        'password' => sha1(trim(strtoupper('1234'))),
        'activo' => '1',
    ...
    redirect(base_url('usuarios/ver/'.$id_inserted));
}
...
function registrar_cliente() {
        ...
        $data = array(
            'nombre' => trim(strtoupper($this->input->post('nombre'))),
            'telefono' => trim(strtoupper($this->input->post('telefono'))),
            'correo' => trim(strtoupper($this->input->post('correo'))),
            'rfc' => trim(strtoupper($this->input->post('rfc'))),
            'dir_fiscal' => trim(strtoupper($this->input->post('dir_fiscal'))),
            'num_exterior' => trim(strtoupper($this->input->post('num_exterior'))),
            'num_interior' => trim(strtoupper($this->input->post('num_interior'))),
            'colonia' => trim(strtoupper($this->input->post('colonia'))),
            'ciudad' => trim(strtoupper($this->input->post('ciudad'))),
            'codigo_postal' => trim(strtoupper($this->input->post('codigo_postal'))),
        );

        $this->usuarios_model->crear_cliente($data);

        redirect(base_url('usuarios/catalogo'));
    }
    function subirfoto() {
    ...
    function ajax_getClientes(){
        $query = "SELECT * from clientes where 1 = 1";// having Name like '%$texto%'";
...
function new_pass()
    {
        $pass=sha1(trim(strtoupper($this->input->post('pass'))));
        $id=$this->input->post('id');
        $array = array('password' => $pass, );
        $res = $this->Conexion->modificar('usuarios', $array, null, array('id' => $id));
        
            redirect(base_url('usuarios/ver/'.$id));
