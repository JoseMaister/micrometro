        ...
        success: function(result) {
            if(result)
            {
                ...
                $.each(rs, function(i, elem){
                    ...
                    ren.insertCell(4).innerHTML = "<button type='button' onclick='mdlModificar(this)' value=" + elem.id + " class='btn btn-warning btn-xs'><i class='fa fa-pencil'></i> Editar</button><button tipe='button' onclick='eliminar(this)' value=" + elem.id + " class='btn btn-danger btn-xs'><i class='fa fa-trash'></i> Eliminar</button>";
                });
        ...
        success: function(result) {
            if(result)
            {
                ...
                $.each(rs, function(i, elem){
                    ...
                    var op = new Option(elem.bajo_a + ' - ' + elem.alto_a, elem.id);
                    $('#opPrecios').append(op);
                });
    ...
    $.ajax({
        ...
        success: function(result) {
            if(result)
            {
                ...
                $('#opMagnitud').val(rs.magnitud);

                $('#lblCodigo').text('000'+id);
                ...
                if(rs.sitio == 'OS/LAB')
                {
                 ...
                 else if (rs.sitio == 'REF'){
                    $('#cbRef').iCheck('check');
                }
                 else if (rs.sitio == 'TAM'){
                    $('#cbTaMa').iCheck('check');
                }
...
function agregar(){
    if(validacion())
    {
        ...
        servicio.id = 0;
        servicio.magnitud = $('#opMagnitud').val();
        ...
        else if($('#cbRef').is(':checked'))
        {
            servicio.sitio = 'REF';
        }
        else if($('#cbTaMa').is(':checked'))
        {
            servicio.sitio = 'TAM';
        }
        else
...
function validacion()
{
    ...
    if(!$('#cbOS').is(':checked') && !$('#cbLab').is(':checked')&& !$('#cbRef').is(':checked')&& !$('#cbTaMa').is(':checked'))
    {
