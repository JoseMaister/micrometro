var TOTAL;
var venta;
function load() {
   total;
}
function mdlVenta() {
    $("#mdlVenta").modal();
    $("#btnCerrar").hide();
}
function mdlAnticipo() {
    $("#mdlAnticipo").modal();
    $("#btnCerrar").hide();
}
function mdlverAnticipo(id) {
      var URL = base_url + "/ventas/ajax_getAnticipos";
    $('#tblAnticipos tbody tr').remove();
 var total=0;
    
     $.ajax({
            type: "POST",
            url: URL,
            data: { id: id},
            success: function(result) {
                if(result)
                {
                    var tab = $('#tblAnticipos tbody')[0];
                    var rs = JSON.parse(result);
                    $.each(rs, function(i, elem){
                        var ren = tab.insertRow(tab.rows.length);
                    ren.insertCell().innerHTML = "$ "+elem.adelanto;
                    ren.insertCell().innerHTML = "$ "+elem.restante;
                    ren.insertCell().innerHTML = elem.user;
                    ren.insertCell().innerHTML = elem.fecha;
                    total=elem.total;

                    });
                    $('#lblTotal').text("Total $ "+total);
                     $("#mdlVerAnticipo").modal();
                }
            },
            error: function(data){
                new PNotify({ title: 'ERROR', text: 'Error', type: 'error', styling: 'bootstrap3' });
                console.log(data);
            },
        });   
}

function  cobrar() {
    var total = document.getElementById('total').textContent;
    var monto = $('#monto').val();
    total=total.substring(8);
    if (monto < total) {

        var r=total-monto;
      $("#total").text("Total: $"+r);
    
    }else if(total => monto){
        var re = monto - total;
        $("#cambio").text("Cambio: $"+re);
         $("#btnCombrar").hide();
         $("#btnCerrar").show();


    }
}
function cobrar_anticipo(id) {
    
    var tipo =document.getElementById("tipo_anticipo").value;
    var monto=$("#monto_anticipo" ).val();
    var total_anticipo = document.getElementById('total_anticipo').textContent;
    var total_anticipo=total_anticipo.substring(8);

     var URL = base_url + 'ventas/ajax_cobrar_anticipo';
    $.ajax({
            type: "POST",
            url: URL,
            data: { tipo:tipo, monto:monto, total_anticipo:total_anticipo, id:id},
            success: function(result) {
                if(result)
                {   
                    window.location.href = base_url + 'ventas/ver_venta/'+id;

                }
            },
        });
}

function cerrar_compra(id) {
    
    var tipo =document.getElementById("tipo").value;
    var total_final = document.getElementById('total').textContent;
    var total_final=total_final.substring(8);
    var descuento = $("input[name=rbDescuento]:checked").val();
    var txt_desc=$( "#txtDescuento" ).val();
    var tipo_descuento=$("#tipo_descuento" ).val();

     var URL = base_url + 'ventas/ajax_cerrar_venta';
    $.ajax({
            type: "POST",
            url: URL,
            data: { tipo:tipo, id:id, total_final:total_final, descuento:descuento, txt_desc:txt_desc, tipo_descuento:tipo_descuento},
            success: function(result) {
                if(result)
                {     

                  window.open(base_url + 'ventas/ticket/'+result, '_blank');

                    //window.location.href = base_url + 'ventas/ticket/'+result;

                }
            },
        });
}



function descuento(){
    var descuento = $("input[name=rbDescuento]:checked").val();
    var total = document.getElementById('total').textContent;


    total=total.substring(8);
    var des=0;
    var total_final=0;
    if (descuento == "otro") {
        $("#descuento").show()

    }else{
        $("#descuento").hide();
         desc = descuento/100;
        desc = desc * TOTAL;
        total_final = TOTAL-desc;
         $("#total").text("Total: $"+total_final);
        //alert(total_final);
    }
}
function aplicar_descuento() {

    var des=0;
    var total_final=0;

    var tipo=$("#tipo_descuento" ).val();
    var txt_desc=$( "#txtDescuento" ).val();
    

    if (tipo=="efectivo") {
        total_final = TOTAL-txt_desc;
         $("#total").text("Total: $"+total_final);

    }else if (tipo=="porcentaje") {
         desc = txt_desc/100;
        desc = desc * TOTAL;
        total_final = TOTAL-desc;
         $("#total").text("Total: $"+total_final);

    }
}
function modalAsignarCliente(){
    $("#mdlClientes").modal();
}
function buscarCliente(){
    var URL = base_url + "/ventas/ajax_getClientes";
    $('#tblBuscarClientes tr').remove();
    var cliente = $("input[name=rbCliente]:checked").val();
    var texto = $("#txtBuscarCliente").val();
    texto = texto.trim();
    
    if(texto)
    {
        $.ajax({
            type: "POST",
            url: URL,
            data: { texto: texto, cliente:cliente },
            success: function(result) {
                if(result)
                {
                    var tab = $('#tblBuscarClientes tbody')[0];
                    var rs = JSON.parse(result);
                    $.each(rs, function(i, elem){
                        var ren = tab.insertRow(tab.rows.length);
                        var cell_id = ren.insertCell(0);
                        cell_id.style.display = "none";
                        cell_id.innerHTML = elem.id;
                        var cell = ren.insertCell(1);
                        cell.innerHTML = elem.nombre;
                        cell.style.width = "70%";
                        ren.insertCell(2).innerHTML = "<button type='button' onclick='infoCliente(this)' class='btn btn-default btn-xs' value=" + elem.id + "><i class='fa fa-info-circle'></i> Info </button> <button type='button' onclick='asignarCliente(this)' class='btn btn-primary btn-xs' data-empresa=" + elem.nombre + " value=" + elem.id + "><i class='fa fa-plus'></i> Asignar</button>";
                    });
                }
            },
            error: function(data){
                new PNotify({ title: 'ERROR', text: 'Error', type: 'error', styling: 'bootstrap3' });
                console.log(data);
            },
        });
    }

}


function infoCliente(btn){
    var id = $(btn).val();
    var URL = base_url + "ventas/ajax_getCliente"
    $.ajax({
        type: "POST",
        url: URL,
        data: { id: id },
        success: function(result) {
            if(result)
            {
                var rs = JSON.parse(result);

                $("#cliente").html(rs.nombre);
                $("#lstTelefono").html("Telefono: "+rs.telefono);
                $("#lstCorreo").html("Correo: "+rs.correo);
                $("#lstRFC").html("RFC: "+rs.rfc);
                $("#lstDire").html("Direccion Fiscal: "+rs.dir_fiscal);                
                $('#mdlInfo').modal();
            }
        },
        error: function(data){
            new PNotify({ title: 'ERROR', text: 'Error', type: 'error', styling: 'bootstrap3' });
            console.log(data);
        },
    });
}
function asignarCliente(btn){
    var id= $(btn).val();
   
    
    var URL = base_url + "ventas/ajax_setCliente";
   //alert(CURRENT_QR);

    $.ajax({
        type: "POST",
        url: URL,
        data: { venta : venta, id: id },
        success: function(result) {
            if(result)
            {
               window.location.reload();

            }
        },
        error: function(data){
            new PNotify({ title: 'ERROR', text: 'Error', type: 'error', styling: 'bootstrap3' });
            console.log(data);
        },
    });
}